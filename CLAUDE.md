...
3. Embeds the latest user message via Gemini `text-embedding-004` (768-d).
4. Checks the in-memory semantic cache (LRU 100, cosine ≥ 0.92, TTL 24h). On hit, streams the cached answer word-by-word.
5. On miss, retrieves top-5 chunks from `index.json` (loaded once at boot) and calls `gemini-2.0-flash` with the system prompt + context. Streams deltas as SSE (`event: delta` / `event: done` / `event: error`).
